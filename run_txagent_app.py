...
os.environ["VLLM_USE_V1"] = (
    "0"  # Disable v1 API for now since it does not support logits processors.
)
...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Accordion("Settings", open=False):
        ...
        max_round_slider = gr.Slider(
            minimum=0, maximum=50, step=1, value=default_max_round, label="Max round"
        )
        ...
        with gr.Accordion(
            "⚙️ Settings", open=False, visible=False
        ) as protected_accordion:
            with gr.Row():
