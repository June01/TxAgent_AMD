...
class TxAgent:
    def __init__(
        ...
        tool_content_summary_threshold=2000000,
        ...
        server_port=8890,  # vLLM server port
        server_host="localhost",  # vLLM server host
    ):
    ...
    def load_models(self, model_name=None):
        ...
        # For compatibility, we still need tokenizer for some operations
        # This will be loaded from the server when needed
        self.chat_template = None  # Will be set when we get tokenizer info
        self.tokenizer = None  # Will be loaded from server when needed

        return f"Model {model_name} connected to server at {self.server_host}:{self.server_port}."
        ...
        if function_call_json is not None:
            ...
            for i in range(len(function_call_json)):
                ...
                if function_call_json[i]["name"] == "Finish":
                    ...
                    # Estimate token count (rough approximation since we don't have direct tokenizer access)
                    token_count = len(str(call_result).split()) * 1.3
                    # ==================== MODIFICATION START ====================
                    ...
                    if token_count > self.tool_content_summary_threshold:
                        ...
                        summarized_token_count = len(summarized_result.split()) * 1.3
    ...
    def build_logits_processor(self, messages, llm=None):
        # Note: Logits processing is now handled by the vLLM server
        # This method is kept for compatibility but doesn't return processors
        # The server-side vLLM will handle repetition avoidance if configured
        return None
    ...
    def llm_infer(
        ...
        for msg in messages:
            ...
            # Only add tool_calls if it's properly formatted and not a string
            if "tool_calls" in msg and msg["tool_calls"]:
                tool_calls = msg["tool_calls"]
                # Skip if tool_calls is a string (like "[]") or malformed
                if isinstance(tool_calls, list) and len(tool_calls) > 0:
                    # Validate that tool_calls contains proper dictionaries
                    valid_tool_calls = []
                    for tc in tool_calls:
                        if isinstance(tc, dict) and "name" in tc and "arguments" in tc:
                            valid_tool_calls.append(tc)
                    if valid_tool_calls:
                        api_msg["tool_calls"] = valid_tool_calls
            api_messages.append(api_msg)

        # Add output_begin_string if provided
        if output_begin_string is not None:
            if api_messages and api_messages[-1]["role"] == "user":
                api_messages[-1]["content"] += output_begin_string
            else:
                api_messages.append({"role": "user", "content": output_begin_string})

        # Check token status if required
        token_overflow = False
        if check_token_status and max_token is not None:
            # Estimate token count (rough approximation)
            total_text = " ".join([msg["content"] for msg in api_messages if isinstance(msg.get("content"), str)])
        ...
        try:
            ...
            #     # logprobs=True,
            #     # top_logprobs=10,
            #     # n=1,  # Single response for compatibility
            #     tools=api_tools,
            #     # extra_body={
            #     #     "top_k": -1,
            #     #     "vllm_xargs": {
            ...
            # Only add tools if they exist and server supports them
            if api_tools:
            ...
            if choice.message.content:
                ...
                # Handle tool calls - convert to text format for compatibility
                tool_calls_text = []
                for tool_call in choice.message.tool_calls:
                    tool_calls_text.append(f'{{"name": "{tool_call.function.name}", "arguments": {tool_call.function.arguments}}}')
            ...
            print(f"Error in llm_infer: {e}")
            # If tool calls fail, try without tools
            if api_tools and ("tool choice" in str(e).lower() or "tool-call-parser" in str(e).lower()):
